...
#define DRAW_PROGRESS_NR_IF_NOT_DONE(nr) do { if (!IS_FIRST_RUN_DONE()) { lcd_lib_draw_stringP((nr < 10) ? 106 : 100, 0, PSTR( #nr "/3")); } } while(0)
#define DRAW_PROGRESS_NR(nr) do { lcd_lib_draw_stringP((nr < 10) ? 106 : 100, 0, PSTR( #nr "/3")); } while(0)
#define CLEAR_PROGRESS_NR(nr) do { lcd_lib_clear_stringP((nr < 10) ? 106 : 100, 0, PSTR( #nr "/3")); } while(0)
...
static void ZUnlimitedDefaultSettings()
{
    ...
    // max acceleration
    enquecommand_P(PSTR("M201 X3000 Y3000 Z100 E10000"));
...
void lcd_menu_zulimited_reset()
{
    lcd_question_screen(NULL, ZUnlimitedDefaultSettings, PSTR("RESET"), NULL, lcd_change_to_previous_menu, PSTR("CANCEL"));
    lcd_lib_draw_string_centerP(10, PSTR("RESET to default"));
    lcd_lib_draw_string_centerP(20, PSTR("settings for"));
    lcd_lib_draw_string_centerP(30, PSTR("z-unlimited"));
    lcd_lib_update_screen();
}
...
static void lcd_menu_first_run_init_2()
{
    ...
    lcd_lib_draw_string_centerP(30, PSTR("Z-Unlimited, we make"));
    lcd_lib_draw_string_centerP(40, PSTR("some adjustments"));
    lcd_lib_update_screen();
}
...
static void zunlimited_firstrun_done()
{
    SET_FIRST_RUN_DONE();
    ZUnlimitedDefaultSettings();
    menu.return_to_main();
}
...
{
    SELECT_MAIN_MENU_ITEM(0);
    lcd_info_screen(NULL, zunlimited_firstrun_done, PSTR("CONTINUE"));
    DRAW_PROGRESS_NR_IF_NOT_DONE(3);
    lcd_lib_draw_string_centerP(10, PSTR("New homing offset"));
    lcd_lib_draw_string_centerP(20, PSTR("Other accelerations"));

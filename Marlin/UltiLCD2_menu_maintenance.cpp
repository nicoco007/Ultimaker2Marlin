...
{
    lcd_tripple_menu(PSTR("UNLIMITED|DEFAULTS"), PSTR("ADVANCED"), PSTR("RETURN"));
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_MAIN(0))
        {
            ...
            menu.add_menu(menu_t(lcd_menu_zulimited_reset));
        }
...
static void lcd_advanced_item(uint8_t nr, uint8_t offsetY, uint8_t flags)
{
    ...
    if (nr == index++)
        ...
        strcpy_P(buffer, PSTR("Move Z"));
    else if (nr == index++)
        strcpy_P(buffer, PSTR("Move Z fast"));
    else if (nr == index++)
        strcpy_P(buffer, PSTR("Home Z"));
    else if (nr == index++)
...
#endif
    else if (nr == EXTRUDERS + BED_MENU_OFFSET + 8)
    {
...
{
    lcd_scroll_menu(PSTR("ADVANCED"), BED_MENU_OFFSET + EXTRUDERS + 10, lcd_advanced_item, lcd_advanced_details);
    if (lcd_lib_button_pressed)
    {
        uint8_t index = 0;
        ...
        {
            lcd_lib_keyclick();
            menu.add_menu(menu_t(lcd_menu_maintenance_zmove, 0));
        }
        else if (IS_SELECTED_SCROLL(index++))
        {
            lcd_lib_keyclick();
            menu.add_menu(menu_t(lcd_menu_maintenance_zmove1, 0));
        }
        else if (IS_SELECTED_SCROLL(index++))
        {
            lcd_lib_keyclick();
...
void lcd_menu_maintenance_zmove()
{
    ...
    char buffer[16];
    float_to_string2(current_position[Z_AXIS], buffer, PSTR(" mm"));
    lcd_lib_draw_string_center(30, buffer);
    lcd_lib_update_screen();
}
